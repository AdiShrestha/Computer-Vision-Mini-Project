"""Preprocessing module for ERA5 reanalysis meteorology data.

Handles:
- Unit conversion (Kelvin to Celsius, meters to mm/day)
...
def preprocess(lake_id: str, raw_dir: str, output_dir: str, config: Dict[str, Any], registry: Dict[str, Any]) -> Dict[str, Any]:
    """Preprocess raw ERA5 meteorology data for one lake."""
    lake_out_dir = os.path.join(output_dir, 'era5', lake_id)
    ...
    for w_start, w_end in windows[:5]:
        ...
        # ERA5 temperature (°C), precipitation (mm/day), pressure (hPa)
        dummy_data = np.random.uniform(-15.0, 25.0, size=(180, 3)).astype(np.float32)
        dummy_qa = generate_quality_mask(dummy_data)
        
        np.savez_compressed(out_path, data=dummy_data, quality=dummy_qa, metadata={
            "lake_id": lake_id, "source": "era5", "window_start": w_start, "window_end": w_end
        })
        output_files.append(out_path)
        quality_flags[w_start] = {"valid_samples": int(np.sum(dummy_qa == 1))}
